...
void Mesh::addFace(Point3LL& v0, Point3LL& v1, Point3LL& v2, int color)
{
...
{
    this->aabb_ = mesh.aabb_;
    this->mesh_name_ = mesh.mesh_name_;
    this->settings_.copy(mesh.settings_);
}

void Mesh::setExtruderNr(int extruder_nr)
{
    auto s = std::to_string(extruder_nr);
    this->settings_.add("wall_extruder_nr", s);
    this->settings_.add("wall_0_extruder_nr", s);
    this->settings_.add("wall_x_extruder_nr", s);
    this->settings_.add("roofing_extruder_nr", s);
    this->settings_.add("top_bottom_extruder_nr", s);
    this->settings_.add("infill_extruder_nr", s);
}
