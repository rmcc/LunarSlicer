...
{
    if (color_segments_set_.size() <= 1 && color_segments_set_.find(MESH_NO_PAINTING_COLOR) != color_segments_set_.end()) {
        return false;
    }
    return true;
}
...
void SlicerLayer::makePolygons(const Mesh* mesh)
{
    ...
    if (!mesh->settings_.get<bool>("colorful_slicing_enable") || !hasColoredSegments()) {
        segments_.clear();
    }
...
Slicer::Slicer(
    ...
    : mesh(i_mesh)
{
    const bool colorful_slicing_enable = mesh->settings_.get<bool>("colorful_slicing_enable");
        ...
        [&](auto layer_it)
        {
            ...
            for (unsigned int face_idx = 0; face_idx < mesh.faces_.size(); face_idx++)
            {
                ...
                */
                ...
                layer.color_segments_set_.insert(face.color);
            }
        });
}
    ...
    const std::vector<AdaptiveLayer>* adaptive_layers)
{
    ...
    for (LayerIndex layer_nr = 1; layer_nr < slice_layer_count; layer_nr++)
    {
        ...
        layers_res[layer_nr - 1].max_z_ = layers_res[layer_nr].z_;
        layers_res[layer_nr].min_z_ = layers_res[layer_nr - 1].z_;
    }
    layers_res.back().max_z_ = layers_res.back().z_ + thickness;
...
void Slicer::buildColorFace(const Mesh& mesh, std::vector<std::pair<int32_t, int32_t>>& zbbox, std::vector<SlicerLayer>& layers)
{
    cura::parallel_for(layers,
                       [&](auto layer_it) {
                           SlicerLayer& layer = *layer_it;
                           const int32_t& min_z = layer.min_z_;
                           const int32_t& max_z = layer.max_z_;

                           for (int i = 0; i < mesh.faces_.size(); ++i)
                           {
                               if (mesh.faces_[i].color <= MESH_NO_PAINTING_COLOR) {
                                   continue;
                               }
                               ...
                               layer.color_faces_idx_.emplace_back(i);
                           }
                       });
}
