...
void PolygonUtils::splitToSimplePolygons(Shape& polygons, std::vector<Shape>& out)
{
    polygons.simplify();
    if (polygons.size() <= 1) {
    ...
    for (int i = 0; i < index_area_sorts.size(); ++i)
    {
        if (index_area_sorts[i].second > 0) {
            out.emplace_back();
            out.back().push_back(polygons[index_area_sorts[i].first]);
        } else {
            bool is_add = false;
            for (int j = 0; j < out.size(); ++j)
            {
               Point2LL p = polygons[index_area_sorts[i].first].front();
               if(out[j].inside(p)) {
                    out[j].push_back(polygons[index_area_sorts[i].first]);
                    is_add = true;
                    break;
               }
            }
...
Shape PolygonUtils::simplifyByScale(Shape& polygons, coord_t offset)
{
